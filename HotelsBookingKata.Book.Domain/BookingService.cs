...
{
    public BookingOperationResultDto Book(
        string employeeId,
        string hotelId,
        string roomType,
        DateTime checkIn,
        DateTime checkOut,
        out BookingDto? bookingDto
    )
    {
        bookingDto = null;
        if (checkIn >= checkOut)
            return new CheckOutDateIsNotLaterThanCheckInDto();
        var roomsInHotel = hotelService.GetNumberOfRoomsByTypeAndHotel(hotelId, roomType);
        if (roomsInHotel == 0)
            return new HotelDoesNotHaveRoomTypeDto();
        if (
            bookingRepository
                .GetAll()
                .Count(booking =>
...
public class CheckOutDateIsNotLaterThanCheckInDto()
    : BookingOperationResultDto(
        "Booking unsuccessful because checkout date is later than check in"
    );

public class HotelDoesNotHaveRoomTypeDto()
    : BookingOperationResultDto("Hotel does not have room type");

public class RoomTypeIsNotAllowedDto() : BookingOperationResultDto("Room type is not allowed");

public class NoFreeRoomDto()
    : BookingOperationResultDto("Room type is booked for the hotel for the selected dates");
